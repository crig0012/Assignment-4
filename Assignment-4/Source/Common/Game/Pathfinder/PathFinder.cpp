...
{
    //Saftey check that the state is idle, if it isnt then the path is either
    //already found and in the process of being
    ...
    //Safety check that the destination tile and the current tile indexes arent the same
    ...
    //Alocate the current tiles path node and add it to the open list
...
void PathFinder::update(double aDelta)
{
    ...
    //Is the state searching
	//Changing the while to an if will slow down the algorithm and help with debugging
    while(isSearchingPath() == true && m_DestinationTileIndex != -1)
    {
        //Safety check that there is acyually something in the open lise
        ...
        {
            //Build the final node path, this will use the current nodes parents
        ...
        //If we got here that means we havent reached our destination. We need to
        //get the adjacent tiles from the node and add the, to the open list
        ...
        for(int i = 0; i < adjacentTiles.size(); i++)
        {
            ...
            //Does the tile existin the closed list?
            ...
            {
                //If it doesnt create a new path node for the tile
                ...
                int hScore = getManhattanDistanceCost(adjacentTile, destinationTile);
                adjacentNode->setScoreH(hScore);
            ...
            else
            {
                ...
                //If it does exist in the open list compare the scores and
                //keep the one with the lower Fscore, which really means
                //keep the one with the lower G score, since the H score
                //will be identical
